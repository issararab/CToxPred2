...
def smiles_batch_to_graph(smiles_list: List[str]) -> Tuple[np.ndarray, np.ndarray]:
    """
    ...
    for smiles in smiles_list:
        ...
        for atom in smiles_mol.GetAtoms():
            smiles_atoms_features_temp.append(_atom_feature(atom))  ### atom features
        mol_dim = min(len(smiles_atoms_features_temp), max_num_atoms)
        smiles_atoms_features[:mol_dim,] = smiles_atoms_features_temp[:mol_dim]
        features_matrices.append(smiles_atoms_features)
        # Adj-preprocessing
        smiles_adj[:mol_dim, :mol_dim] = smiles_adj_temp[:mol_dim, :mol_dim] + np.eye(
            mol_dim
        )
    ...
    for i, array in enumerate(adjacency_matrices):
        padded_adjacency_matrices[i, :, :target_size] = np.pad(
            array, ((0, 0), (0, target_size - array.shape[1])), mode="constant"
        )
...
def _atom_feature(atom: Atom) -> np.ndarray:
    ...
    return np.array(
        ...
        + _one_of_k_encoding(atom.GetDegree(), [0, 1, 2, 3, 4, 5, 6])
        + _one_of_k_encoding(atom.GetTotalNumHs(), [0, 1, 2, 3, 4])
        + _one_of_k_encoding(atom.GetImplicitValence(), [0, 1, 2, 3, 4, 5])
        + [atom.GetIsAromatic()]
        + _get_ring_info(atom)
    )
