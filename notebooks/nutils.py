...
def _split_list_to_columns(row):
...
def _resolve_2d_structure(smiles_string):
...
def _generate_predictions_ssl(smiles_list: List[str]) -> Dict[str, any]:
    ...
    # Process hERG
    LOGGER.info("Predict hERG toxicity!")
    ...
    # Process Nav1.5
    LOGGER.info("Predict Nav1.5 toxicity!")
    ...
    # Process Cav1.2
    LOGGER.info("Predict Cav1.2 toxicity!")
...
def _generate_predictions_sl(smiles_list: List[str]) -> Dict[str, any]:
    ...
    ## Load model
    hERG_predictor = hERGClassifier(1905, 2, GLB_DROPOUT_RATE)
    path = ["..", "CToxPred2", "models", "model_weights", "hERG", "_herg_checkpoint.model"]
    hERG_predictor.load(os.path.join(*path))
    device = torch.device("cpu")
    all_predictions = torch.empty(len(hERG_fingerprints), 2, GLB_FORWARD_ITERATIONS)
    ...
    ## Load model
    nav15_predictor = Nav15Classifier(2453, 2, GLB_DROPOUT_RATE)
    ...
    all_predictions = torch.empty(len(nav15_features), 2, GLB_FORWARD_ITERATIONS)
    ...
    ## Load model
    cav12_predictor = Cav12Classifier(2586, 2, GLB_DROPOUT_RATE)
    ...
    all_predictions = torch.empty(len(cav12_features), 2, GLB_FORWARD_ITERATIONS)
...
def process_smiles(smiles_list, GLB_Model = "rf-ssl"):
    ...
    properties_df = base_frame.apply(_split_list_to_columns, axis=1)
    ...
    base_frame["_2d_structure"] = base_frame.apply(
        lambda x: _resolve_2d_structure(x["SMILES"]), axis=1
    )
    if GLB_Model == "dl-sl":
